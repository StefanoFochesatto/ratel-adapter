...
# Open log file for coupling data
log_file = open("coupling_data.log", "w")
log_file.write(f"{'Time [s]':>12} | {'Step':>6} | {'Max Rel Disp':>15}\n")
log_file.write("-" * 40 + "\n")
...
while precice.is_coupling_ongoing():
    ...
    max_disp = np.max(np.abs(u_delta.vector().get_local()))
    
    log_line = f"{t:12.4f} | {n:6d} | {max_disp:15.6e}"
    print(f"[Solid] {log_line}")
